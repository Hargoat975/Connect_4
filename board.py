...
square_height = square_size * (board_height+2)
    ...
    for column in range(board_width):
        for row in range(board_height):
            ...
            if board[row][column] == 0:
                ...
                pygame.draw.circle(screen,RED, (int(column*square_size+square_size/2), int(row*square_size+square_size+square_size/2)), radius)
            elif board[row][column] == -1:
                pygame.draw.circle(screen,YELLOW, (int(column*square_size+square_size/2), int(row*square_size+square_size+square_size/2)), radius)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            pygame.draw.rect(screen, BLACK, (0,0, square_width, square_size))
            posx = event.pos[0]
            if turn % 2 == 1:
        ...
        pygame.display.update()
        if not gameOver(sample):
            if event.type == pygame.MOUSEBUTTONDOWN:
                if turn % 2 == 1:
                    posx = event.pos[0]
                    col = int(math.floor(posx/square_size))
                    if valid_move(sample, col):
    ...
    if gameOver(sample): 
        ...
        if check_winner(sample) == -1:
            ...
            screen.blit(label, (40,10))
        if tie(sample):
            label = myFont.render("Tie", 1, RANDOM)
            screen.blit(label, (40,10))
        pygame.display.update()
